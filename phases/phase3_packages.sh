...
echo "Pre-flight Check: Verifying wlan1 connection..."
echo "------------------------------------------------"
log_step "Pre-flight check: verifying wlan1 connection"
...
if ip link show wlan1 &>/dev/null; then
    ...
    log_success "wlan1 interface exists: $WLAN1_STATE"
else
    print_error "NOT FOUND"
    log_error "wlan1 interface not found"
    print_warning "Phase 2 (Internet Connection) may not have completed successfully"
    exit 1
fi
...
if [ -n "$WLAN1_IP" ]; then
    ...
    log_success "wlan1 has IP: $WLAN1_IP"
else
    print_error "NO IP ADDRESS"
    log_error "wlan1 has no IP address"
    print_error "Phase 2 connection was lost. Please re-run Phase 2."
    exit 1
fi

# Check wpa_supplicant process (from Phase 2 manual connection)
echo -n "  wpa_supplicant process... "
if pgrep -f "wpa_supplicant.*wlan1" >/dev/null; then
    print_success "running (from Phase 2)"
    log_success "wpa_supplicant is running for wlan1"
else
    print_warning "not running"
    log_warning "wpa_supplicant not running for wlan1"
fi

# Note about service
echo -n "  wlan1-internet service... "
if systemctl is-enabled --quiet wlan1-internet.service 2>/dev/null; then
    print_success "enabled (will start on boot)"
    log_success "wlan1-internet.service is enabled for boot"
else
    print_warning "not enabled"
    log_warning "wlan1-internet.service not enabled"
fi
