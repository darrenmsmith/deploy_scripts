...
ExecStop=/usr/bin/pkill -f 'dhcpcd.*wlan1'
...
if [ -f "$WPA_CONF" ] && grep -q "network=" "$WPA_CONF"; then
    print_info "Existing WiFi config found, checking validity..."

    # Check if it has required headers, if not add them
    if ! grep -q "ctrl_interface" "$WPA_CONF"; then
        print_warning "Config missing headers, fixing..."

        # Save existing network block
        sudo grep -A 10 "network=" "$WPA_CONF" > /tmp/wpa_network.tmp
...
WPAEOF
        ...
        # Append saved network block
        sudo cat /tmp/wpa_network.tmp >> "$WPA_CONF"
        sudo rm /tmp/wpa_network.tmp

        print_success "WiFi config fixed with proper headers"
    else
        print_success "Valid WiFi config already exists"
    fi
else
    # No config exists, need credentials
    print_warning "No WiFi config found, need credentials"
...
if systemctl is-active --quiet wlan1-wpa.service; then
    ...
    print_error "wpa service failed"
...
if systemctl is-active --quiet wlan1-dhcp.service; then
    ...
    print_error "dhcp service failed"
